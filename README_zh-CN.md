...
1. 服务器为离线模式。在登录阶段（HELLO），服务器发送自定义登录查询（transactionId + 标识 `trueauth:auth` + 随机 nonce）。
...
- /trueauth link <name>
    ...
    - 子命令示例：
        - /trueauth link dryrun <name>  — 预览操作，不写盘
        - /trueauth link run <name>     — 执行迁移（示例实现中默认会先备份）
    - 行为：
...
备份目录：
- world/backups/trueauth/<时间戳>/<name>/
...
- 成品在：`build/libs/trueauth-<version>.jar`
